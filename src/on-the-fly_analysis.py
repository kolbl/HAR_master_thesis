import json
import requests
from HAR_classification import *
import numpy as np
...
class Sensor_Emulator(HAR_classification):
    def __init__(self):
...
def on_the_fly():
    ...
    for i in range(40):
        time_stamp.append(i)
        test_sample =  test_samples(i + 1)
        x_test, y_test = test_sample.iloc[:, :-1], test_sample.iloc[:, -1]
        y_pred = model.predict(x_test)

        print("The new data point is predicted to be of class {}".format(y_pred)) # todo: map integer to text label.
