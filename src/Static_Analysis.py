...
class Machine_Learn_Static(object):
    ...
    def evaluate_LSTM_model(self, train_X, train_y, test_X, test_y):
        ...
        # TODO: use class weights as extra input to deal with the class imbalance?
        # TODO: grid search/ random search
...
def dataset_generator_classification():
    # read a dataset
    ...
    # TS fresh
    # extracted_features = extract_features(training_samples, column_id="TIMESTAMP")
    # print(extracted_features)


    test_samples = test_samples.drop(["TIMESTAMP"], axis=1)
    # test_samples = test_samples.drop(["Kettle Sensor"], axis=1)
    test_samples = test_samples.astype(float)

    training_samples = training_samples.drop(["TIMESTAMP"], axis=1)
    # training_samples = training_samples.drop(["Kettle Sensor"], axis=1)
    ...
    #
    # parameters = {'kernel': ('linear', 'poly', 'rbf', 'sigmoid'), 'C': [1, 10], 'decision_function_shape': ('ovo', 'ovr'), 'random_state': [0, 1]}
    # svc = SVC()
    ...
    # parameters = {'solver': ('lbfgs', 'adam'), 'activation': ('identity', 'logistic', 'tanh', 'relu'), 'early_stopping': (True, False)}
    # nn = MLPClassifier(max_iter=500)
    # clf = GridSearchCV(nn, parameters, cv=5)
    # print(clf.fit(x_train, y_train))
    # print(sorted(clf.cv_results_.keys()))
    ...
    # # print(x_train.head(2).to_dict(orient="records"))
    # yseq = map(lambda x: str(x), y_train.values.tolist())
    #
    # crf.fit(x_train.to_dict(orient="records"), yseq)
    # return
    ...
    ### CNN with LSTM - set time steps to 127
    # # [samples, timesteps, features per step]
    # # reshape data into time steps of sub-sequences for CNN with LSTM!!
    # n_steps, n_length = 4, 32
    # print("shit shape is ", x_train.shape[0])
    # train_X = x_train.values.reshape((x_train.shape[0], n_steps, n_length, n_features))
    # test_X = x_test.values.reshape((x_test.shape[0], n_steps, n_length, n_features))
    ##########
    ...
    # p = {
    #  'first_neuron': [4, 16, 64, 128, 256],
    #  'second_neuron': [4, 16, 64, 128, 256],
    #  'dropout': [0.0, 0.5, 0.5],
    #  'pool_size': [2],
    ...
    #
    #
    # h = talos.Scan(train_X, train_Y, model=classification.evaluate_CNN_model_talos, params=p, print_params=True, dataset_name="HAR", experiment_no='3', shuffle=False, val_split=0.2)
    #
    #
    ...
    for r in range(repeats):
        loss, accuracy, rmse, recall, precision, f1, mcc = classification.evaluate_LSTM_model(train_X, train_Y, test_X, test_Y)
        # loss, accuracy, rmse, recall, precision, f1, mcc = classification.evaluate_CNN_model(train_X, train_Y, test_X, test_Y)
        # loss, accuracy, rmse, recall, precision, f1, mcc = classification.evaluate_CNN_LSTM_model(train_X, train_Y, test_X, test_Y)
    ...
    # TODO: Best Activity: Plot actual vs. predicted activities (2 bars with amount of activities next to each other, per activity)
    # TODO: balanced_accuracy_score per class per algorithm
    # TODO: precision recall curves with tensorboard: https://github.com/tensorflow/tensorboard/blob/master/tensorboard/plugins/pr_curve/README.md
...
if __name__ == '__main__':
    ...
    dataset_generator_classification()
